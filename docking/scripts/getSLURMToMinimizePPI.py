...
#SBATCH --job-name minimize.0.3A\n\
#SBATCH --partition=project\n\
#SBATCH --qos=maxjobs\n\
#SBATCH --output=/bgfs01/insite/taras.dauzhenka/data/6HAX/results/constraint.harmonic.1/job_0/ternary_0.3A/minimized/minimize_%A_%a.out\n\
#SBATCH --error=/bgfs01/insite/taras.dauzhenka/data/6HAX/results/constraint.harmonic.1/job_0/ternary_0.3A/minimized/minimize_%A_%a.err\n\
#SBATCH --array=1-2\n\
#SBATCH --cpus-per-task=2\n\n"
        ...
        for file1 in os.listdir( inputFolder ):
            if "_fixed.pdb" == file1[ len( file1 ) - 10 : len( file1 ) ]:
                ...
                paramsFile  =  file1.split( "_fixed.pd" )[ 0 ] + "_lig.params"
                paramsFilePath  =  os.path.join( inputFolder, paramsFile )
                logFile  =  file1.split( ".pd" )[ 0 ] + ".log"
                logFilePath  =  os.path.join( inputFolder, logFile )
                scoreFile  =  file1.split( ".pd" )[ 0 ] + ".score"
        ...
        out.write( line )
        out.write( "\n" )
        out.write( "cd /bgfs01/insite/taras.dauzhenka/data/6HAX/results/constraint.harmonic.1/job_0/ternary_0.3A/minimized\n" )
        ...
        out.write( cmd )
        out.write( "cd /bgfs01/insite/taras.dauzhenka/data/6HAX/\n" )
        out.write( "\nwait\necho\necho \"All done.\"\n" )
