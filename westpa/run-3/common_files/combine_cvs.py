...
f3=sys.argv[3]  # input,  VHL_Protac.dat
f4=sys.argv[4]  # output, CV1.dat
...
smarca_contacts=[0,0]   # want these to be just the Smarca-related contacts, required for deciding what the cv is supposed to be
contacts=[0,0]          # want these to be the Smarca-PROTAC-VHL protected-residue contacts
...
with open(f1) as f1:                            # deal with Smarca-VHL interactions
    ...
    if len(s1)>2:                               # i.e., if iter>1, thus invoked by '../westpa_scripts/runseg.sh'
        ...
        contacts[0]+=(int(x1[1])+int(x1[2]))    # add to contacts 
        mindist[0]=float(x1[3])                 # the min. distance
    x1=s1[-1].strip().split()                   # now the current values
    contacts[1]+=(int(x1[1])+int(x1[2]))        # add to contacts 
...
smarca_contacts[0]=contacts[0]
smarca_contacts[1]=contacts[1]

with open(f3) as f1:                            # deal with VHL-Protac interactions    
    s1=f1.readlines()
    if len(s1)>2:                               # i.e., if iter>1, thus invoked by '../westpa_scripts/runseg.sh'
        x1=s1[-2].strip().split()               # parent's values, not current one
        contacts[0]+=(int(x1[1])+int(x1[2]))    # add to contacts
